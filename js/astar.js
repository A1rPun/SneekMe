...
SneekMe.astar = (function () {
    ...
    astar.prototype.setAcceptableTiles = function (acceptableTiles) {
        this.acceptableTiles = acceptableTiles;
    };

    astar.prototype.setGrid = function (level) {
        this.grid = level;
    ...
    astar.prototype.ManhattanDistance = function (Point, Goal) {	// linear movement - no diagonals - just cardinal directions (NSEW)
        return abs(Point.x - Goal.x) + abs(Point.y - Goal.y);
    };

    astar.prototype.DiagonalDistance = function (Point, Goal) {	// diagonal movement - assumes diag dist is 1, same as cardinals
        return max(abs(Point.x - Goal.x), abs(Point.y - Goal.y));
    };

    astar.prototype.EuclideanDistance = function (Point, Goal) {	// diagonals are considered a little farther than cardinal directions
    ...
    astar.prototype.Neighbours = function (x, y) {
        var N = y - 1,
            S = y + 1,
            E = x + 1,
            W = x - 1,
    ...
    astar.prototype.DiagonalNeighbours = function (myN, myS, myE, myW, N, S, E, W, result) {
        if (myN) {
    ...
    astar.prototype.DiagonalNeighboursFree = function (myN, myS, myE, myW, N, S, E, W, result) {
        myN = N > -1;
...
// returns boolean value (world cell is available and open)
    astar.prototype.canWalkHere = function (x, y) {
        return ((this.grid[y] !== undefined) &&//redundant?
                (this.grid[y][x] !== undefined) &&//redundant?
                (~this.acceptableTiles.indexOf(this.grid[y][x])));
    };

    //TODO: fix findpath when pathend is on 0,0 -- actual bug?
    // this returns an array of coordinates
    // that is empty if no path is possible
    astar.prototype.findPath = function (pathStart, pathEnd) {        
